      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-bocra-navy to-[#163158] py-9 px-4">
        <div className="max-w-[980px] mx-auto">
          <div className="text-xs text-white/45 mb-2">
            <Link href="/" className="underline hover:text-white/70 transition-colors">
              Home
            </Link>{" "}
            / Portals
          </div>

          <h1 className="text-xl md:text-[26px] font-bold text-white mb-1.5">
      ...
      <section className="max-w-[980px] mx-auto px-4 md:px-5 py-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {portalItems.map((item) => (
          <Link
            key={item.title}
            href={item.href}
            className="bg-white rounded-card border border-gray-100 p-5 transition-all duration-300 hover:shadow-card-hover hover:scale-[1.02]"
          >
