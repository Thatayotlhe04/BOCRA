...
- `ADMIN_API_KEY` (optional but recommended for protected status update endpoint)
...
- `npm run test` — automated smoke checks
...
- Basic API rate limiting is implemented for complaint submission, lookup, and status updates.
- Complaint evidence uploads support signed upload URLs and metadata tracking.
- Complaint status updates can be protected with `ADMIN_API_KEY`.

## 10) Quality Gates

- Smoke tests are located at `scripts/smoke-tests.sh` and run with `npm run test`.
- CI workflow runs tests and production build on push/pull request (`.github/workflows/ci.yml`).
- Deployment troubleshooting checklist: `docs/DEPLOYMENT_TROUBLESHOOTING.md`.
