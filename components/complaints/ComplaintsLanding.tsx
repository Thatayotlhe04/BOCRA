...
function FAQ({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div onClick={() => setOpen(!open)} className="bg-white rounded-[10px] border border-gray-100 mb-2 cursor-pointer overflow-hidden transition-all hover:border-gray-200">
      <div className="px-4 py-3.5 flex justify-between items-center gap-3">
        ...
        <span className="shrink-0 flex transition-transform duration-200" style={{ transform: open ? "rotate(180deg)" : "none" }}><ChevDownIcon color="#ADB5BD" /></span>
      </div>
      {open && <div className="px-4 pb-4 text-sm text-gray-600 leading-relaxed border-t border-gray-100 pt-3 -mt-0.5">{a}</div>}
    </div>
  );
}
...
export default function ComplaintsLanding() {
  ...
  const handleTrack = () => {
    ...
    if (!id.startsWith("CMP-")) {
      setTrackError("Complaint IDs start with CMP- (e.g. CMP-2026-0341)");
      return;
    }
    // Navigate to tracker — it will handle the lookup
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* File New */}
        <div onClick={() => router.push("/complaints/new")}
          ...
          <div className="w-[52px] h-[52px] rounded-[14px] bg-bocra-magenta-light flex items-center justify-center mx-auto mb-3.5"><ComplaintIcon color="#E31B6D" size={26} /></div>
          <h3 className="text-[17px] font-bold text-gray-900 mb-1.5">File New Complaint</h3>
          <p className="text-[13px] text-gray-500 mb-4 leading-relaxed">Submit a complaint against a telecom, broadcasting, or postal provider</p>
          <span className="inline-flex items-center gap-1.5 bg-bocra-magenta text-white px-5 py-2 rounded-lg text-[13px] font-semibold">Start Filing <ArrowRightIcon color="#fff" size={14} /></span>
        </div>

        {/* Track */}
        <div className="bg-white rounded-card p-6 md:p-7 border border-gray-100 border-t-[3px] border-t-bocra-navy text-center">
          <div className="w-[52px] h-[52px] rounded-[14px] bg-gray-100 flex items-center justify-center mx-auto mb-3.5"><SearchIcon color="#0C2340" size={26} /></div>
          <h3 className="text-[17px] font-bold text-gray-900 mb-1.5">Track Complaint</h3>
          <p className="text-[13px] text-gray-500 mb-4 leading-relaxed">Check the status of your existing complaint</p>
          <div className="flex gap-2">
            <input value={trackId} onChange={(e) => { setTrackId(e.target.value); setTrackError(""); }}
          ...
          {trackError && <div className="text-bocra-magenta text-xs mt-2 font-medium">{trackError}</div>}
          <div className="mt-3 text-[11px] text-gray-400">
            Demo: <button onClick={() => router.push("/complaints/track/CMP-2026-0341")} className="text-bocra-blue underline">CMP-2026-0341</button> or <button onClick={() => router.push("/complaints/track/CMP-2026-0298")} className="text-bocra-blue underline">CMP-2026-0298</button>
          </div>
        </div>
      </div>
