...
const mockData: Record<string, TrackingData> = {
  "CMP-2026-0341": {
    ...
    status: "investigating", filed: "8 Mar 2026", lastUpdate: "2 hours ago",
    steps: [
  ...
  "CMP-2026-0298": {
    ...
    status: "resolved", filed: "1 Mar 2026", lastUpdate: "5 days ago",
    steps: [
...
export default function ComplaintTracker({ id }: { id: string }) {
  ...
  if (!data) {
    return (
      <div className="text-center py-16">
        <div className="w-16 h-16 rounded-full bg-bocra-magenta-light flex items-center justify-center mx-auto mb-4">
          <ShieldIcon color="#E31B6D" size={28} />
        </div>
  ...
  return (
    <>
      <Link href="/complaints" className="inline-flex items-center gap-1 mb-6 text-[13px] text-gray-500 font-medium hover:text-bocra-blue transition-colors">
        ← Back to Complaints
      </Link>
      ...
      <div
        ...
        style={{ background: isResolved ? "linear-gradient(135deg, #00A651, #00C064)" : "linear-gradient(135deg, #0C2340 0%, #11385A 60%, #0A4568 100%)" }}
      >
        ...
        <div className="relative z-10">
          <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                {!isResolved && (
                  <span className="relative flex h-3 w-3">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-bocra-yellow opacity-60" />
                    <span className="relative inline-flex rounded-full h-3 w-3 bg-bocra-yellow" />
                  </span>
                )}
              </div>
            ...
            <div className="text-right">
              <div className="inline-block px-4 py-1.5 rounded-full text-xs font-bold tracking-wide" style={{ background: isResolved ? "rgba(255,255,255,0.2)" : "rgba(255,209,0,0.2)", color: "#fff" }}>
          ...
          <div className="mb-2">
            <div className="flex justify-between text-xs text-white/35 mb-2">
              <span>Progress</span>
              <span className="font-mono">{doneCount}/{data.steps.length}</span>
            </div>
            <div className="w-full h-2.5 bg-white/10 rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all duration-1000 ease-out" style={{
                width: `${(doneCount / data.steps.length) * 100}%`,
                background: isResolved ? "rgba(255,255,255,0.5)" : "linear-gradient(90deg, #FFD100, #FFC000)",
              }} />
            </div>
          </div>
          ...
          {currentStep && !isResolved && (
            <div className="flex items-center gap-3 mt-5 bg-white/[0.07] rounded-xl px-4 py-3.5 border border-white/[0.06]">
              <div className="w-9 h-9 rounded-full bg-bocra-yellow/20 flex items-center justify-center shrink-0">
                <ClockIcon color="#FFD100" size={17} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-white">{currentStep.label}</div>
                <div className="text-xs text-white/40 leading-relaxed">{currentStep.desc}</div>
              </div>
          ...
          {isResolved && (
            <div className="flex items-center gap-3 mt-5 bg-white/[0.12] rounded-xl px-4 py-3.5">
              ...
              <div>
                <div className="text-sm font-semibold text-white">Case Closed Successfully</div>
                <div className="text-xs text-white/45">Resolution accepted — {data.steps[data.steps.length - 1].time}</div>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
        <div className="px-6 md:px-8 pt-6 pb-1 border-b border-gray-50">
          <h3 className="text-base font-bold text-gray-900 mb-0.5">Complaint Timeline</h3>
          <p className="text-xs text-gray-400 mb-3">Every action is logged and visible to you</p>
        </div>
          ...
          {data.steps.map((step, i) => {
            ...
            return (
              <div
                ...
                style={{ opacity: isVisible || !step.done ? 1 : 0, transform: isVisible ? "translateY(0)" : "translateY(8px)" }}
              >
                {/* Dot + line */}
                <div className="flex flex-col items-center">
                  {step.current && !isResolved ? (
                    <div className="w-10 h-10 rounded-full bg-bocra-blue/10 flex items-center justify-center shrink-0">
                      <span className="relative flex h-4 w-4">
                        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-bocra-blue opacity-40" />
                        <span className="relative inline-flex rounded-full h-4 w-4 bg-bocra-blue" />
                      </span>
                    </div>
                  ) : step.done ? (
                    <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-all duration-500" style={{ background: isVisible ? (isResolved ? "#E8F9EF" : "#EAF5FD") : "#F1F3F5" }}>
                      <div className="w-5 h-5 rounded-full flex items-center justify-center transition-all duration-500" style={{ background: isVisible ? lineColor : "#DEE2E6" }}>
                  ...
                  {!isLast && (
                    <div className="w-0.5 flex-1 min-h-[24px] my-1.5 rounded-full transition-all duration-700" style={{
                      background: step.done && data.steps[i + 1]?.done ? lineColor
                        : step.done && (data.steps[i + 1]?.current) ? `linear-gradient(to bottom, ${lineColor}, #EDEFF2)`
                        : "#EDEFF2",
                    }} />
                  )}
                </div>
                ...
                <div className={`flex-1 min-w-0 ${isLast ? "pb-0" : "pb-5"}`}>
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="text-[13px] font-semibold" style={{ color: step.done || step.current ? "#1A1D21" : "#CED4DA" }}>
                        {step.label}
                      </div>
                      <div className="text-[12px] mt-0.5 leading-relaxed" style={{ color: step.done || step.current ? "#495057" : "#DEE2E6" }}>
                        {step.desc}
                      </div>
                    </div>
                    <div className="text-[11px] font-medium whitespace-nowrap mt-0.5 shrink-0" style={{ color: step.done ? "#868E96" : "#DEE2E6" }}>
                      {step.time}
                    </div>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-5">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-card">
          ...
          <p className="text-sm text-gray-500 leading-relaxed mb-3">
            Questions about your complaint? Contact BOCRA Consumer Affairs.
          </p>
          <div className="flex flex-col gap-1.5 text-sm">
            <span className="text-gray-700 font-medium">+267 395 7755</span>
            <span className="text-bocra-blue font-medium">complaints@bocra.org.bw</span>
          </div>
        </div>
      </div>
    </>
